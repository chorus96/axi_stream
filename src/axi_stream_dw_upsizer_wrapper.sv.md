# axi_stream_dw_upsizer_wrapper.sv

`axi_stream_dw_upsizer`의 AMD 커스텀 IP 패키징용 flat-port 래퍼. Vivado IP 패키징 요구사항에 맞게 struct/interface 포트를 모두 `logic` 신호로 대체하고, `parameter type`을 정수 파라미터로 교체한다.
...
|----------------|--------------|---------|-----------------------------------------------|
| `DataWidthIn`  | int unsigned | 8       | 입력 데이터 폭 (bits); `DataWidthOut`보다 작아야 함 |
| `DataWidthOut` | int unsigned | 32      | 출력 데이터 폭 (bits); `DataWidthIn`의 정수배  |
...
| `s_axis_tdata`   | input  | **DataWidthIn**  | 입력 데이터 (좁은 버스)          |
...
| `m_axis_tdata`   | output | **DataWidthOut** | 출력 데이터 (넓은 버스)           |
...
flowchart LR
  ...
  subgraph axi_stream_dw_upsizer_wrapper
    direction LR
    RS["flat → in_req\nin_rsp → s_axis_tready"]
    US["axi_stream_dw_upsizer\n────────────────────\nFSM: AcceptDataIn\n     Pad\n     DataOut\nshift accumulator\ncounter"]
  ...
  Slave -->|DataWidthIn| RS --> US --> TM -->|DataWidthOut| Master
```
...
3. `axi_stream_dw_upsizer` 코어 인스턴스
4. `out_req` struct 필드 → flat `m_axis_*` 출력 신호 assign

### 사용 예시

```
DataWidthIn=8, DataWidthOut=32 인 경우:
  입력 4 beats (8-bit × 4) → 출력 1 beat (32-bit)
  처리량: 출력 측 처리량 = 입력 측 처리량 / 4
  tlast 조기 도달 시 나머지 슬롯은 0으로 자동 패딩
```
