...
/// AMD custom IP wrapper for axi_stream_dw_downsizer.
/// Replaces interface/struct ports with flat logic signals for Vivado IP packaging.
module axi_stream_dw_downsizer_wrapper #(
  /// AXI Stream Data Width In (bits); must be a multiple of DataWidthOut
  parameter int unsigned DataWidthIn  = 32,
  /// AXI Stream Data Width Out (bits); must be less than DataWidthIn
  ...
  // Slave (RX) AXI4-Stream port — wide input
  ...
  // Master (TX) AXI4-Stream port — narrow output
...
endmodule : axi_stream_dw_downsizer_wrapper
